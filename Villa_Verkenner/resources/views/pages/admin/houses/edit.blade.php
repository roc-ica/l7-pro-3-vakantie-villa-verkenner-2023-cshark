...
<div class="admin-dashboard">
    ...
    <div class="form-container">
        <form action="{{ route('admin.houses.update', $house) }}" method="POST" enctype="multipart/form-data" class="admin-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="price" name="price" value="{{ old('price', $house->price) }}" required>
                    @error('price')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="rooms" name="rooms" value="{{ old('rooms', $house->rooms) }}" required>
                    @error('rooms')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        <option value="Beschikbaar" {{ old('status', $house->status) == 'Beschikbaar' ? 'selected' : '' }}>Available</option>
                        <option value="Verkocht" {{ old('status', $house->status) == 'Verkocht' ? 'selected' : '' }}>Sold</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="popular" name="popular" required>
                        <option value="0" {{ old('popular', $house->popular) == '0' || old('popular', $house->popular) === 0 ? 'selected' : '' }}>Not Featured</option>
                        <option value="1" {{ old('popular', $house->popular) == '1' || old('popular', $house->popular) === 1 ? 'selected' : '' }}>Featured</option>
                    </select>
                ...
                <div class="form-group full-width">
                    ...
                    @if($house->image)
                        <div class="current-image">
                            <img src="{{ asset('storage/' . $house->image) }}" alt="{{ $house->name }}" width="200">
                            <p>Current Image</p>
                        </div>
                    @endif
